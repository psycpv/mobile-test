...
export default function useNotificationHistory() {
  ...
  const getLocalStorageTimestamp = () => localStorage.getItem('lastUpdateTimestamp')
  const setCurrentTimestamp = useCallback(() => localStorage.setItem('lastUpdateTimestamp', currentTimestamp()), [])
  ...
  useEffect(() => {
    ...
    const timestamp = localStorageTimestamp === null ? currentTimestamp() : localStorageTimestamp
