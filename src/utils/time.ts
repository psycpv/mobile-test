...
export function formatTimestampUTC_date(time?: string, format?: string): string | null {
  if (time === undefined) return null
  return dayjs.utc(Number(time) * 1000).format(format ?? 'MMMM D, YYYY')
}

export function formatTimestamp(timeLength: number, format?: string): string {
  return dayjs(timeLength).format(format ?? 'MMM DD, YYYY - HH:mm:ss')
}
export function formatTimestampHistory(timeLength: number, format?: string): string {
  return dayjs(timeLength).format(format ?? 'MMM DD, YYYY - HH:mm')
}
