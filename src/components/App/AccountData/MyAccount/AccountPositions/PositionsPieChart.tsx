...
import BASED from '/public/static/images/etc/SimpleLogo.svg'
...
const renderActiveShape = (props: any) => {
  ...
  const { cx, cy, midAngle, innerRadius, outerRadius, startAngle, endAngle, fill, percent, name, color } = props
  ...
  return (
    <g>
      ...
      <text x={ex + (cos >= 0 ? 1 : -1) * 12} y={ey} textAnchor={textAnchor} fill={color} style={{ fontSize: '12px' }}>
        {name}
      </text>
      <text
        x={ex + (cos >= 0 ? 1 : -1) * 12}
...
export default function PositionsPieChart() {
  ...
  const data = marketQuotesInfo.map((quoteInfo) => ({
    ...
    color: quoteInfo.positionType === PositionType.LONG ? theme.green1 : theme.red1,
  ...
  return loading ? (
    <ChartWrapper>
      <CenterWrapper>
        <Image src={BASED} width={shapeDim.width} height={shapeDim.height} alt={'Based'} />
      </CenterWrapper>
      <ResponsiveContainer>
        <PieChart>
          <Pie
