...
const calculationSeperatePattern = RegExp(`([$%]?\\d+(?:\\.\\d+)?)(?:\\s*([-+])\\s*([$%]?\\d+(?:\\.\\d+)?))?`)
...
export function calculateString(
  ...
  const matchString = input.match(calculationSeperatePattern)
  let result = ''
  if (matchString) {
    result = calculateOverall(matchString[1], matchString[2], matchString[3], balance, pricePrecision, basePrice)
  }
  return result
}
