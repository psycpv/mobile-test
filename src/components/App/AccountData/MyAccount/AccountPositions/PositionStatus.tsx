...
import { useTotalNotionalValue } from 'hooks/usePositionOverview'
...
function PositionStatusBody() {
  ...
  const { totalPositionNumber, longPositionNumber, shortPositionNumber } = getPositionNumbers(marketQuotesInfo)
  ...
  return (
    <>
      ...
      <PositionTotalValue>
        <PositionNumber>${formatPrice(totalPositionValue, 2, true)}</PositionNumber>
...
export default function PositionStatus() {
  ...
  return <Container>{loading ? <EmptyPositionStatusBody /> : <PositionStatusBody />}</Container>
}
